...
export default function EditableBlock({
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLDivElement>) {
    if (slashOpen && (e.key === "Enter" || e.key === "ArrowDown" || e.key === "ArrowUp" || e.key === "Escape")) {
    ...
    if (e.key === "Enter") {
      e.preventDefault();
      const continuesType: BlockType[] = ["bulleted", "numbered", "todo", "toggle"];
      if (block.content.trim() === "" && block.type !== "paragraph" && block.type !== "code") {
        changeBlockType(pageId, block.id, "paragraph");
        return;
      }
      const nextType = continuesType.includes(block.type) ? block.type : "paragraph";
  ...
  function handleDrop(e: React.DragEvent) {
    ...
    if (draggedId && draggedId !== block.id) {
      reorderBlock(pageId, draggedId, block.id, dragOver === "bottom" ? "after" : "before");
    }
    setDragOver(null);
    setDragId(null);
  }
...
function BlockBody({
  ...
  switch (block.type) {
    case "heading1":
      return <div {...editableProps} className={editableProps.className + " text-3xl font-bold pt-2"} />;
    case "heading2":
      return <div {...editableProps} className={editableProps.className + " text-2xl font-semibold pt-1.5"} />;
    case "heading3":
      return <div {...editableProps} className={editableProps.className + " text-xl font-semibold pt-1"} />;
    case "bulleted":
      return (
        <div className="flex items-start gap-2">
          <span className="mt-0.5 select-none leading-6">•</span>
          <div {...editableProps} className={editableProps.className + " leading-6"} />
        </div>
      );
    case "numbered":
      return (
    ...
    case "quote":
      return (
        <div className="border-l-[3px] border-ink-light pl-3 dark:border-ink-dark">
          <div {...editableProps} className={editableProps.className + " italic leading-6"} />
        </div>
      );
    case "callout":
      return (
    ...
    case "code":
      return (
        <div className="rounded-md bg-[#f7f6f3] p-3 font-mono text-sm dark:bg-[#2c2c2c]">
          <div {...editableProps} className={editableProps.className + " whitespace-pre-wrap"} />
        </div>
      );
    case "divider":
      return (
    ...
    case "paragraph":
    default:
      return <div {...editableProps} className={editableProps.className + " leading-6"} />;
  }
}
