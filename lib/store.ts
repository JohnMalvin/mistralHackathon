...
interface Store extends WorkspaceState {
  ...
  reorderBlock: (pageId: string, fromId: string, toId: string, position: "before" | "after") => void;
    ...
    (set, get) => ({
      ...
      createPage: (parentId = null, title = "Untitled") => {
        ...
        set((s) => {
          ...
          const rootIds = parentId ? s.rootIds : [...s.rootIds, page.id];
      ...
      restorePage: (id) => {
        set((s) => {
          const pages = { ...s.pages };
          if (pages[id]) pages[id] = { ...pages[id], isDeleted: false };
          return { pages };
        });
      },
      ...
      permanentlyDeletePage: (id) => {
        set((s) => {
          ...
          if (parentId && pages[parentId]) {
            pages[parentId] = {
              ...pages[parentId],
              children: pages[parentId].children.filter((cid) => cid !== id),
            };
          }
          return { pages, rootIds };
        });
      ...
      duplicatePage: (id) => {
        ...
        if (!original) return;
        const copy = makePage(original.parentId, original.title + " (copy)");
        copy.blocks = original.blocks.map((b) => ({ ...b, id: nanoid(10) }));
        ...
        set((state) => {
          ...
          if (original.parentId && pages[original.parentId]) {
            pages[original.parentId] = {
              ...pages[original.parentId],
              children: [...pages[original.parentId].children, copy.id],
            };
          }
      ...
      updatePageCover: (id, color) =>
        set((s) => ({
          pages: { ...s.pages, [id]: { ...s.pages[id], coverColor: color } },
        })),
      ...
      toggleFavorite: (id) =>
        set((s) => ({
          pages: {
            ...s.pages,
            [id]: { ...s.pages[id], isFavorite: !s.pages[id].isFavorite },
          },
        })),
      ...
      movePage: (id, newParentId, index) =>
        set((s) => {
          ...
          if (oldParentId && pages[oldParentId]) {
            pages[oldParentId] = {
              ...pages[oldParentId],
              children: pages[oldParentId].children.filter((c) => c !== id),
            };
          } else {
      ...
      addBlock: (pageId, afterBlockId, type = "paragraph", content = "") => {
...
export function seedWorkspaceIfEmpty() {
  ...
  useStore.setState((state) => {
    ...
    page.icon = "👋";
    page.blocks = [
      { id: nanoid(10), type: "heading1", content: "Welcome to your workspace" },
      {
        id: nanoid(10),
        type: "paragraph",
        content:
